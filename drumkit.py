...
def generate_rimshot(sample_rate: int = 44100) -> np.ndarray:
    """Hyper-short, high-frequency pop simulating a stick hitting a drum rim."""
    duration = 0.03
    t = np.linspace(0, duration, int(sample_rate * duration), endpoint=False)
    
    # High frequency pop
    phase = 2 * np.pi * 1400.0 * t
    amp_env = np.exp(-110.0 * t)
...
def export_kit_coherence_test(bpm=120, bars_to_loop=4, sample_rate=44100, filename="kit_coherence_test.wav"):
    ...
    kit = {
        ...
        5: generate_tom(120.0, sample_rate),  # Low Tom
        6: generate_tom(220.0, sample_rate),  # High Tom
        7: generate_rimshot(sample_rate)
    }
    ...
    pattern_matrix = np.array([
        ...
        [1,  0,  0,  0,  0,  0,  0,  0],  # Step 0: Downbeat Kick + Hat + Rim accent
        [1,  0,  0,  0,  0,  0,  0,  0],  # Step 1: Up-hat
        [1,  1,  0,  0,  0,  0,  0,  0],  # Step 2: Backbeat Snare + Layered Clap
        [1,  0,  0,  0,  0,  0,  0,  0],  # Step 3: Low Tom roll
        [1,  0,  0,  0,  0,  0,  0,  0],  # Step 4: Mid-bar Kick + Open Hat breathing
        [1,  0,  0,  0,  0,  0,  0,  0],  # Step 5: High Tom syncopation
        [1,  1,  0,  0,  0,  0,  0,  0],  # Step 6: Second Snare punch
        [1,  0,  0,  0,  0,  0,  0,  0]   # Step 7: Final Open Hat tail
    ])

    print("🎛️ Mixing grid matrices into unified time slices...")
    ...
    print(f"🔄 Cloning pattern across {bars_to_loop} sequential loops...")
